...
parser.add_argument('--data_input', type=str, default="train_da_0.p,train_da_1.p,dev.p,test.p") # comma separated pkl files 
...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(F.relu(self.conv2(x))) # 128x8
        x = self.pool(F.relu(self.conv3(x))) # 64x16
        x = self.pool(F.relu(self.conv4(x))) # 32x16
        x = self.dropout(x)   
        x = x.view(-1, 32*32*16)
...
plt.savefig("simple_train_dev_acc_{0}_{1}.png".format(args.num_epochs, args.suffix))
...
plt.savefig("simple_train_dev_loss_{0}_{1}.png".format(args.num_epochs, args.suffix))
